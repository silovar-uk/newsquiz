...
interface ImportPanelProps {
  onImport: (quizSet: QuizSet) => Promise<void>;
}
...
export function ImportPanel({ onImport }: ImportPanelProps) {
  ...
  const save = async () => {
    ...
    setIsSaving(true);
    try {
      await onImport(validation.quizSet);
      setMessage(`「${validation.quizSet.title}」を取り込みました。ホームからすぐに始められます。`);
      setText('');
      setValidation(null);
    } catch (error) {
    ...
    <main className="page-shell">
      ...
      <section className="import-card">
        ...
        <div className="import-footer">
          ...
          <button className="primary-button" onClick={save} disabled={!validation?.valid || isSaving}>{isSaving ? '取り込み中…' : 'このセットを取り込む'}</button>
        </div>
      </section>
      ...
      {validation && (
        <section className={`validation-card ${validation.valid ? 'validation-ok' : 'validation-error'}`}>
          <div className="validation-title"><span>{validation.valid ? '✓' : '!'}</span><div><h2>{validation.valid ? '取り込み可能' : '修正が必要'}</h2><p>{validation.valid ? '形式は問題ありません。警告だけ確認して取り込んでください。' : '下記のエラーを直すと取り込めます。'}</p></div></div>
